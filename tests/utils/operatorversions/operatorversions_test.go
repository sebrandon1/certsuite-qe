...
func TestExtractShortVersion(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			result := extractShortVersion(tc.input)
...
func TestGetOperatorConfig(t *testing.T) {
	testCases := []struct {
		...
		},
		{
			...
			expectedCertifiedPkg:   "mongodb-enterprise", // Alternative for 4.20
			expectedCommunityPkg:   "grafana-operator",
			expectedLightweightPkg: "postgresql",
			expectedUncertifiedPkg: "cockroachdb",
		},
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			config := GetOperatorConfig(tc.ocpVersion)

			assert.Equal(t, tc.expectedCertifiedPkg, config.CertifiedOperator.PackageName)
			assert.Equal(t, tc.expectedCommunityPkg, config.CommunityOperator.PackageName)
			assert.Equal(t, tc.expectedLightweightPkg, config.LightweightOperator.PackageName)
			assert.Equal(t, tc.expectedUncertifiedPkg, config.UncertifiedOperator.PackageName)
		})
	}
}
...
func TestGetCertifiedOperator(t *testing.T) {
	...
	op := GetCertifiedOperator("4.19")
	assert.Equal(t, "cockroachdb-certified", op.PackageName)
	assert.Equal(t, CatalogCertifiedOperators, op.CatalogSource)

	// 4.20 should use alternative certified operator (mongodb-enterprise)
	op = GetCertifiedOperator("4.20")
	assert.Equal(t, "mongodb-enterprise", op.PackageName)
	assert.Equal(t, CatalogCertifiedOperators, op.CatalogSource)
}

func TestGetCommunityOperator(t *testing.T) {
	// Should be same across all versions
	op := GetCommunityOperator("4.14")
	assert.Equal(t, "grafana-operator", op.PackageName)

	op = GetCommunityOperator("4.20")
	assert.Equal(t, "grafana-operator", op.PackageName)
}

func TestGetLightweightOperator(t *testing.T) {
	// Should be same across all versions
	op := GetLightweightOperator("4.14")
	assert.Equal(t, "postgresql", op.PackageName)

	op = GetLightweightOperator("4.20")
	assert.Equal(t, "postgresql", op.PackageName)
}
...
func TestIsVersion420OrLater(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			result := IsVersion420OrLater(tc.version)
			assert.Equal(t, tc.expected, result)
		})
	}
}
